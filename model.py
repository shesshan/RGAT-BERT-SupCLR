...
class Aspect_Bert_GAT(nn.Module):
    ...
    def forward(self, input_ids, input_aspect_ids, word_indexer, aspect_indexer, input_cat_ids, segment_ids, pos_class, dep_tags, text_len, aspect_len, dep_rels, dep_heads, aspect_position, dep_dirs):
        ...
        feature_output = outputs.last_hidden_state  # (N, L, D)
        pool_out_1 = outputs.pooler_output  # (N, D)
        pool_out_2 = None
        ...
        dep_out = torch.cat(dep_out, dim=1).view(
            batch_size, -1)  # (N, H * D)
        dep_out = self.dep_W(dep_out)  # (N, D)
        feature_out_1 = torch.cat(
            [dep_out, pool_out_1], dim=1)  # (N, D')
        feature_out_2 = None
        # feature_out = gat_out
        #############################################################################################
        ...
        return logits, feature_out_1, feature_out_2
