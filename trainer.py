...
def train(args, train_dataset, model, test_dataset):
    '''Train the model'''
    tb_writer = SummaryWriter(comment='_'+str(args.dataset_name))
    ...
    tr_ce_loss, ce_logging_loss, tr_cl_loss, cl_logging_loss = 0.0, 0.0, 0.0, 0.0
    all_eval_results = []
    model.zero_grad()
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.gat_our:
                ...
                logits, feature_out, _ = outputs
            ce_loss = CELoss(logits, labels)
            cl_loss = CLLoss(feature_out, labels)
            loss = ce_loss + args.alpha_2 * cl_loss
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    tb_writer.add_scalar(
                        'ce_loss', (tr_ce_loss - ce_logging_loss) / args.logging_steps, global_step)
                    #tb_writer.add_scalar('cl_loss', (tr_cl_loss - cl_logging_loss) / args.logging_steps, global_step)
                    logging_loss = tr_loss
                    ce_logging_loss = tr_ce_loss
                    #cl_logging_loss = tr_cl_loss
